...
//		logger.debug(ex);
            ...
            try {
                Map<?, ?> map1 = ComputerMonitor.cpuinfo();
                Thread.sleep(CPUTIME);
                Map<?, ?> map2 = ComputerMonitor.cpuinfo();
    ...
    public static double getMemUsage() {
        if (osName.toLowerCase().contains("windows") || osName.toLowerCase().contains("win")) {
            try {
                ...
                // 剩余的物理内存
                long freePhysicalMemorySize = osmxb.getFreeSwapSpaceSize();
...
//		logger.debug(e);
    ...
    public static void timer(int wait) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //cpu
        double cpuUsage = ComputerMonitor.getCpuUsage();
        //当前系统的内存使用率
        double memUsage = ComputerMonitor.getMemUsage();
        //当前系统的硬盘使用率
        double diskUsage = ComputerMonitor.getDiskUsage();
